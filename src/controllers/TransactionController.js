...
module.exports = {
    async saveTransaction(request, response) {
        try {
            const { type_operation, type_material, price_kg, kgs, type_payment, responsible, wallet_id, total_value } = request.body

            const wallet = await transactionServices.saveTransaction({ type_operation, type_material, price_kg, kgs, type_payment, responsible, wallet_id, total_value })
